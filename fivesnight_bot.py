...
async def create_teams(interaction: discord.Interaction):
    ...
    team_creator = Views.TeamCreationView(organizer=interaction.user)
    ...
    team_one = Helpers.assign_roles(team_creator.team_one)
    team_two = Helpers.assign_roles(team_creator.team_two)
    ...
    if interaction.channel != DEBUG_CHANNEL:
        ...
        if any([not len(team_one), not len(team_two)]):
            ...
            await interaction.edit_original_response(
                ...
                view=Views.DeleteThisMessageView(),
            )
            return
...
async def record_last_outcome(interaction: discord.Interaction):
    ...
    if all([not bot.team_one, not bot.team_two]):
        ...
        return
    record_match_view = Views.RecordLastMatchView()
    await interaction.response.send_message(
...
async def check_group_winrates(interaction: discord.Interaction):
    # Create embed
    ...
    for record in match_manager.get_match_history().items():
        ...
        winrates_embed.add_field(
            ...
            value=f"{games_won}/{games_played} ({Helpers.calc_winrate(games_won, games_played)})",
        )
    ...
    if not len(winrates_embed.fields):
        ...
        return
    # Send winrates embed
    await interaction.response.send_message(embed=winrates_embed, view=Views.DeleteThisMessageView())
